...
const virtualChannels = new URLSearchParams(location.search).has('virtual-channels');
const threadPreview = new URLSearchParams(location.search).has('thread-preview');
const nativeThread = new URLSearchParams(location.search).has('native-thread');
const botThread = new URLSearchParams(location.search).has('bot-thread');
...
if (botThread) addBotThreadFixture();
